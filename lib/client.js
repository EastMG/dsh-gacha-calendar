	...
	factory: (require) => {
		...
		const SOURCES = [
			...
			},
			{
				...
				// 卡池备选来源（设置页下拉可切换；值带 proxy: 前缀表示经 host 代理抓取，fetcher 指定对应抓取器）
				altSources: [
					{
						label: "wiki.gg\uff08\u82F1\u6587\uff09",
			...
			},
			{
				...
				source: "\u5B98\u7F51\u516C\u544A",
				// 经 host 代理 POST 抓取（官网新闻 API，征集名另经小米官方资讯流增强）；版本公告同时含当期卡池与活动
		...
		// 取最新一期「版本更新维护公告」：版本名 / 当期卡池（首位6星角色名，官网无征集名）/ 当期活动 / 维护起止 + 下一期维护日
		...
		// 活动源注册表：条目 → { 默认 + 备选抓取器 }。没有独立活动源的条目（原神/星铁/绝区零/方舟/终末地）
		// 不在表中，活动来源显示"未配置"，活动列保持为空（不再有内置兜底数据）。
		...
		async function fetchEntry(source, signal) {
			...
			try {
				...
				if (source.eventUrl && eventFetcher) {
					...
					if (source.eventUrl === source.url && data && data.event) {
						...
						try {
							...
							const evName = ev && typeof ev.event === "string" ? ev.event : "";
							if (evName) {
								data = data || {};
								data.event = evName;
								data.eventDates = ev && typeof ev.eventDates === "string" ? ev.eventDates : "";
							} else {
								data = data || {};
				...
				} else if (source.eventUrl && source.eventUrl !== source.url && data) {
					// 无解析器的自定义活动源（如原神等未配置活动源的条目自行填写地址）：仅健康检查，不产出活动数据
					try {
						const apiUrl = source.eventUrl + (source.eventUrl.includes("?") ? "&" : "?") + "origin=*";
						const res = await fetch(apiUrl, { signal, headers: { Accept: "application/json" } });
						if (!res.ok) data.eventFail = "event-http-" + res.status;
					} catch (err) {
						data.eventFail = err?.name === "AbortError" ? "aborted" : String(err?.message ?? err).slice(0, 60);
					}
				}
			...
			const doRefresh = (0, react.useCallback)(async () => {
				...
				try {
					...
					let info = result.failed.length === 0
						? `抓取成功 ${okNames.length}/${result.total}${skippedNote}：${okNames.join("、")}`
							...
							games.map((g) => {
								...
								return (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										(0, react_jsx_runtime.jsx)("div", { className: "gacha-cal-cell", title: eventTitle, children: eventName }),
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 6, border: "1px solid var(--dsw-alias-border-l1)", borderRadius: 10, padding: "12px 14px", background: "var(--dsw-alias-bg-layer-1)" }, children: [
						...
						// 表头行（与数据行同 grid 列）
						(0, react_jsx_runtime.jsxs)("div", { style: { display: "grid", gridTemplateColumns: "minmax(90px,1fr) auto minmax(150px,1.3fr) minmax(150px,1.3fr) auto", gap: 10, alignItems: "center", padding: "3px 0", borderBottom: "1px solid var(--dsw-alias-border-l1)", color: "var(--dsw-alias-label-tertiary)", fontSize: 11 }, children: [
							(0, react_jsx_runtime.jsx)("span", { children: "\u6E38\u620F" }),
						...
						sorted.map((g, i) => {
							...
							// 活动来源性质：independent（有活动源，可切换/自定义）/ none（未配置活动源）
							const evKind = g.custom
								? (g.eventUrl ? "independent" : "none")
								: (EVENT_FETCHERS[g.id] ? "independent" : "none");
							return (0, react_jsx_runtime.jsxs)("div", {
								style: { display: "grid", gridTemplateColumns: "minmax(90px,1fr) auto minmax(150px,1.3fr) minmax(150px,1.3fr) auto", gap: 10, alignItems: "center", padding: "4px 0", borderBottom: "1px solid var(--dsw-alias-border-l1)" },
								key: g.id,
								children: [
									...
									// 卡池来源列（选择器在上、自定义输入在下全宽，避免挤压）
									(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 4, justifySelf: "center", minWidth: 0, width: "100%" }, children: [
									...
									// 活动来源列（选择器在上、自定义输入在下全宽；未配置活动源默认进入"自定义"输入）
									(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 4, justifySelf: "center", minWidth: 0, width: "100%" }, children: [
										...
										}) : (0, react_jsx_runtime.jsx)("span", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 11, padding: "3px 6px" }, children: "自定义（未配置）" }),
										(evKind === "independent" && evMode === "custom") || evKind === "none" ? (0, react_jsx_runtime.jsx)("input", {
											type: "text",
											placeholder: evKind === "none" ? "活动源地址（MediaWiki api.php）" : "MediaWiki api.php URL",
					...
					(0, react_jsx_runtime.jsx)("div", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 12 }, children: "数据来源：官方公告 / 官方Wiki（bwiki、PRTS 等）；无联网抓取数据时对应列显示为空。自定义条目只展示你填写的内容，有爬取地址时仅作联网检查。" })
				]
			});
		}
		//#endregion
