	...
	factory: (require) => {
		...
		const SOURCES = [
			...
			},
			{
				...
				via: "proxy",
				url: "https://forum.nexon.com/api/v1/board/3359/threads?alias=bluearchiveTW&countryCode=KR&pageNo=1&paginationType=PAGING&pageSize=50&blockSize=5&hideType=WEB",
				eventUrl: ""
			},
			{
				id: "r1999",
		...
		const PARSERS = {
			...
			endfield: (html) => parseGachaTracker(html)
		};
		// 活动源解析器：eventUrl 抓取的页面用（如鸣潮活动日历 → event/eventDates）
		const EVENT_PARSERS = {
			wuwa: parseWuwaCalendar
		};
		// 直接解析原始 HTML 的解析器（源不是 MediaWiki JSON，如 GachaTracker 页面）
		const RAW_PARSERS = {
			endfield: parseGachaTracker
		};
		//#endregion

		//#region proxy parsers（经 host 同源代理抓取，绕过浏览器 CORS 反爬）
		// 繁体中文时间解析："8月18日(二)維護後" / "9月1日(二)上午9點59分"
		function parseZhTime(raw, nowYear) {
			...
			let h = 0, mi = 0;
			const tm = s.match(/(上午|下午|中午|凌晨|晚上)?\s*(\d{1,2})[點点](\d{1,2})?[分]?/);
			if (tm) {
				let hh = Number(tm[2]);
		...
		// 从公告 summary 提取卡池（期間 + UP 学生）
		function parseBaSummary(summary, nowYear) {
			const s = stripTags(summary);
			const periodM = s.match(/期間\s*[::：]?\s*([^。；\n]+?)\s*[~～\-]\s*([^。；\n]+)/);
			if (!periodM) return null;
			const start = parseZhTime(periodM[1], nowYear);
			const end = parseZhTime(periodM[2], nowYear);
			if (!start || !end) return null;
			const ups = [...new Set([...s.matchAll(/(?:^|\s)(\d{1,2})\)\s*([^\s(（★]+)/g)].map((m) => m[2]))].slice(0, 6);
			return { startTs: start.ts, endTs: end.ts, startText: start.text, endText: end.text, roles: ups.join("、") };
		}

		// 经 host 代理抓取 JSON（fetch 同源 /api/gacha-calendar-proxy）
		async function proxyFetchJson(proxyUrl, referer) {
			const api = "/api/gacha-calendar-proxy?url=" + encodeURIComponent(proxyUrl) + "&referer=" + encodeURIComponent(referer);
		...
		// 蔚蓝档案·国际服：nexon 官方论坛公告板 → 最新招募公告详情 → 当期卡池
		// 两步：1) 列表找招募公告 threadId 2) 详情 summary 提取期間+UP
		async function fetchBaGlobal(listUrl, signal) {
			const ref = "https://forum.nexon.com/bluearchiveTW/";
			const list = await proxyFetchJson(listUrl, ref);
			const threads = Array.isArray(list?.threads) ? list.threads : [];
			// 找最新招募公告（标题含"招募"，置顶优先，列表已按时间倒序）
			const bannerThread = threads.find((t) => /招募/.test(t.title || "") && !/結束|即將/.test(t.title || "")) || threads.find((t) => /招募/.test(t.title || ""));
			if (!bannerThread?.threadId) return null;
			const detail = await proxyFetchJson(`https://forum.nexon.com/api/v1/thread/${bannerThread.threadId}?alias=bluearchiveTW&countryCode=KR`, ref);
			const summary = typeof detail?.summary === "string" ? detail.summary : "";
			if (!summary) return null;
			const parsed = parseBaSummary(summary, new Date().getFullYear());
			if (!parsed) return null;
			const now = Date.now();
			if (!(parsed.startTs <= now && parsed.endTs >= now)) return null;
			// 卡池名：详情标题去掉 ▎ 前缀
			const title = String(detail?.title ?? "").replace(/^▎\s*/, "").trim();
			return {
				banner: title || bannerThread.title,
				version: "",
				roles: parsed.roles,
				bannerDates: `${parsed.startText} ~ ${parsed.endText}`
			};
		}

		// 经代理的解析器（async）：source.via === "proxy"
		const PROXY_PARSERS = {
			"ba-global": (listUrl, signal) => fetchBaGlobal(listUrl, signal)
		};
		//#endregion
		...
		async function fetchGame(source, signal) {
			...
			const rawParser = RAW_PARSERS[source.id];
			const eventParser = EVENT_PARSERS[source.id];
			const proxyParser = PROXY_PARSERS[source.id];
			// 经 host 代理抓取（source.via === "proxy"）：绕过浏览器 CORS/Referer 反爬
			if (source.via === "proxy" && typeof proxyParser === "function" && source.url) {
				try {
					const data = await proxyParser(source.url, signal);
			...
			if (source.url) {
				try {
					...
					if (typeof parser !== "function" && typeof rawParser !== "function") {
						return { ok: true, reason: "reachable" };
					}
					// 原始 HTML 解析器（非 MediaWiki 源）：直接喂 HTML
					if (typeof rawParser === "function") {
						const rawHtml = await res.text();
						const data = rawParser(rawHtml);
						if (!data || !data.banner || !data.bannerDates) return { ok: false, reason: "no-match" };
						return { ok: true, reason: "ok", data };
					}
