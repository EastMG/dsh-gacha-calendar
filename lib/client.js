	...
	factory: (require) => {
		...
		const SOURCES = [
			...
			},
			{
				...
				altSources: [
					{ label: "Bwiki 往期调频", value: ZZZ_BWIKI_URL, fetcher: "zzz-bwiki" }
				],
			...
			},
			{
				...
				altSources: [
					{ label: "Bwiki 角色轮换池", value: WUWA_BWIKI_URL, fetcher: "wuwa-bwiki" }
				],
			...
			},
			{
				...
				altSources: [
					{ label: "PRTS 卡池一览", value: ARKNIGHTS_PRTS_URL, fetcher: "arknights-prts" }
				],
		...
		// 设置页手动切到 PRTS 备选源时则走 CARD_FETCHERS["arknights-prts"]，不经本函数。
		async function fetchArknightsCard(url, signal, now = Date.now()) {
			try {
				...
				if (official.length > 0) {
					...
					return {
						...
						bannerHover: `${p.family ? `${p.family} · ` : ""}限时寻访\n${p.bannerDatesRaw}`
					};
				}
			} catch { /* 官方失败 → 回退 PRTS */ }
		...
		function parseWuwaNotice(list, now = Date.now()) {
			...
			const raw = `${fmt(first.startTs)} ~ ${fmt(first.endTs)}`;
			// 悬停：仅列角色活动唤取，按卡池逐行；各池时间相同则只在末尾写一遍（不同则每行各带时间）
			const sameWin = cur.length > 0 && cur.every((p) => p.startTs === cur[0].startTs && p.endTs === cur[0].endTs);
			const lines = cur.map((p) => {
				const line = `${p.name}·${p.roles || "-"}`;
				return sameWin ? line : `${line}  ${fmt(p.startTs)} ~ ${fmt(p.endTs)}`;
			});
			if (sameWin) lines.push(raw);
			const bannerHover = lines.join("\n");
		...
		async function fetchWuwaCard(entryUrl, signal, now = Date.now()) {
			try {
		...
		async function fetchZzzCard(listUrl, signal, now = Date.now()) {
			try {
		...
		function genericEventPayload(html) {
			...
			const snapshot = items.map((it) => ({ name: it.banner, startTs: it.startTs, endTs: it.endTs, raw: it.rawOriginal || it.raw }));
			const now = Date.now();
		...
		function prtsEventPayload(html) {
			...
			const snapshot = items.map((it) => ({ name: it.banner, startTs: it.startTs, endTs: it.endTs, raw: it.raw }));
			const now = Date.now();
			const active = sortEventItems(snapshot.filter((it) => it.endTs != null && it.endTs >= now && (it.startTs == null || it.startTs <= now)));
			if (active.length === 0) return null;
			const primary = active[0];
		...
		function parseFzWikiActivities(html, now) {
			...
			(function walk(n) {
				...
				if (
					...
					n.type.includes("endfieldCardActivityIndex__activities") &&
					typeof n.attrs.name === "string" &&
					Array.isArray(n.attrs.timeRanges)
				) {
					// 新结构：活动为独立的 __activities 子节点（content 数组内），字段在各自 attrs 上
					const tr = n.attrs.timeRanges[n.attrs.timeRanges.length - 1];
					if (tr && tr.open && tr.close) {
						acts.push({ name: n.attrs.name, tags: n.attrs.tags || [], open: tr.open, close: tr.close });
					}
				}
			...
			// 覆盖当前时刻的活动统一排序（③ 结束时间升序）；外显取第一条，悬停按同序逐行
			const activeActs = sortEventItems(acts
				.filter((a) => parseT(a.open) <= t0 && parseT(a.close) >= t0)
				.map((a) => ({ name: a.name, startTs: parseT(a.open), endTs: parseT(a.close) })));
			if (activeActs.length === 0) return null;
			const primary = activeActs[0];
		...
		function parseSmwActivity(json) {
			...
			for (const [title, r] of Object.entries(results)) {
				...
				if (startTs <= now && endTs >= now) covering.push({ name, startTs, endTs });
			}
			if (covering.length === 0) return null;
			// 统一排序（③）：外显=最快结束的那条，悬停按同序逐行（常驻/长期玩法在排序阶段剔除）
		...
		async function fetchBaGlobal(logListUrl, signal, now = Date.now()) {
			...
			const eventRow = evOrdered[0] || null;
		...
		async function fetchBaCn(listUrl, now = Date.now()) {
			...
			// 当期卡池成员名：卡池名所在那一行里的「（全新）N★（限定）成员“X”」，
			// 如 2、更新限时招募【夏日思绪长…】，…全新3★成员「桔梗（泳装）」、2★成员「莲华（泳装）」登场
			const poolKey = bannerM ? bannerM[1] : (bannerR ? bannerR[1] : "");
			const poolLine = poolKey ? (text.split(/\n+/).find((l) => l.includes(poolKey)) || "") : "";
			const roleNames = [...new Set([...poolLine.matchAll(/(?:\d★)?(?:限定)?成员\s*[「“"]([^」”"]+)[」”"]/g)].map((m) => m[1].trim()).filter(Boolean))];
			const data = {
		...
		async function fetchR99(listUrl, signal, now = Date.now()) {
			...
			const cleanText = (raw) => String(raw)
				...
				.replace(/&hellip;/g, "…").replace(/&times;/g, "×").replace(/&bull;/g, "·")
				.replace(/\n\s*\n+/g, "\n").trim();
			// 单条版本公告的"当期池窗口"：开池=维护结束时刻；结束=下一期维护日前 04:59
			// （公告内"可在X月X日上午5点之后兑换"提示）；无提示按维护日起 +21 天兜底
			const maintWindow = (text) => {
				...
				// 结束日按该公告"自身的维护年份(sm)"推算、跨年补 sm+1 —— 不可用当前年份，
				// 否则往期跨年公告（如 2025/12 的老版本）结束会被推到未来而误判为当期
				let end = new Date(sm, mm - 1, md + 21, 4, 59);
				if (nextM) {
					const base = new Date(sm, mm - 1, md);
					let ey = sm;
					if (new Date(sm, Number(nextM[1]) - 1, Number(nextM[2])) < base) ey = sm + 1;
					end = new Date(ey, Number(nextM[1]) - 1, Number(nextM[2]), 4, 59);
				}
				return { maintM, startTs, endTs: end.getTime() };
			};
			// 窗口匹配：选"版本更新维护公告"中维护窗口覆盖 now 的那一期（不再无条件取最新）
			...
			const endD = new Date(win.endTs);
			const bannerDates = `${fmt(Number(win.maintM[6]), Number(win.maintM[7]), Number(win.maintM[8]), Number(win.maintM[9]), Number(win.maintM[10]))} ~ ${fmt(endD.getFullYear(), endD.getMonth() + 1, endD.getDate(), 4, 59)}`;
		...
		const CARD_FETCHERS = {
			...
			zzz: (url, signal) => fetchZzzCard(url, signal),
			"zzz-bwiki": mkMediaWiki(pickCurrent(parseAllBwiki)),
			arknights: (url, signal) => fetchArknightsCard(url, signal),
			"arknights-prts": mkMediaWiki(selectArknights),
			wuwa: (url, signal) => fetchWuwaCard(url, signal),
			...
			try {
				...
				if (source.eventUrl) {
					...
					if (source.eventUrl === source.url && data && data.event) {
						...
						let evName = "", evDates = "", evDatesRaw = "", evHover = "", gErr = null;
						...
						if (eventFetcher) {
							try {
								...
								evHover = ev && typeof ev.eventHover === "string" ? ev.eventHover : "";
							} catch (err) {
								if (err?.name === "AbortError") throw err;
								gErr = err; // 保留内置抓取器的真实失败原因
							}
						}
						if (!evName) {
							// 通用活动解析仅用于自定义条目（或无内置抓取器）：
							// 内置源失败/未命中时不再尝试浏览器直连兜底——代理源直连必被 CORS 拦成
							// "Failed to fetch"，会掩盖真实原因（解析空/网络/源未收录）
							if (source.custom || !eventFetcher) {
								try {
								...
								} catch (err) { if (!gErr) gErr = err; }
							} else if (gErr === null) {
								gErr = new Error("no-match");
							}
						}
						if (evName) {
							data = data || {};
							...
							if (evHover) data.eventHover = evHover;
						} else {
		...
		// 例外：蔚蓝档案三服的括号是换装版本标识（椿(導覽員)、Shiroko (Cycling)），去掉会与基础版撞名 → 原样保留
		const KEEP_PAREN_ROLE_IDS = ["ba-cn", "ba-global", "ba-jp"];
		function cleanRoleNames(roles, gameId) {
			const keepParen = KEEP_PAREN_ROLE_IDS.includes(gameId);
			return String(roles || "")
				.split(/[、,，]/)
				.map((n) => {
					let s = n.replace(/[「」【】]/g, "");
					if (!keepParen) s = s.replace(/[（(][^）)]*[）)]/g, "");
					s = s.trim();
			...
			const doRefresh = (0, react.useCallback)(async () => {
				...
				try {
					...
					result.results.forEach((r, i) => {
						...
						if (r.ok && r.data) {
							const merged = { ...r.data, okAt: Date.now() };
							// 字段级兜底（统一机制，不针对任何游戏）：
							// 本次成功但某字段为空（如活动源"未收录当期/失败"）时，从上次缓存沿回该字段，
							// 避免"卡池更新成功但活动列闪空/被抹掉"；若沿用旧活动则不再标 eventFail
							const prevRec = prevById[id];
							if (prevRec) {
								if (!merged.banner && prevRec.banner) {
									merged.banner = prevRec.banner;
									merged.roles = prevRec.roles || "";
									if (!merged.bannerDates) merged.bannerDates = prevRec.bannerDates || "";
									if (!merged.bannerDatesRaw) merged.bannerDatesRaw = prevRec.bannerDatesRaw || prevRec.bannerDates || "";
								}
								if (!merged.event && prevRec.event) {
									merged.event = prevRec.event;
									if (!merged.eventDates) merged.eventDates = prevRec.eventDates || "";
									if (!merged.eventDatesRaw) merged.eventDatesRaw = prevRec.eventDatesRaw || prevRec.eventDates || "";
									if (!merged.eventHover) merged.eventHover = prevRec.eventHover || "";
									delete merged.eventFail;
								}
							}
							dataById[id] = merged;
					...
					// 通用提示分类（不针对任何游戏特判）：
					// - 真失败：本次未产出新数据、且此前也没有成功记录（面板无任何可显示内容）→ "失败"
					//   （含源返回 no-match 的情形：无历史可沿用的都算失败，如新加/自定义条目）
					// - 未发布新数据：本次整体未产出但有历史成功（沿用、stale、okAt 不刷新），
					//   或整条成功但活动/卡池列缺失且没有旧值可沿用（部分列未更新）——两者都不算"失败"
					const hardFails = entries.filter((x, i) => !result.results[i]?.ok && !prevById[entries[i].id]);
					const retained = entries.filter((x, i) => !result.results[i]?.ok && prevById[entries[i].id]);
					const partialNoPrev = entries.filter((x, i) =>
						result.results[i]?.ok && result.results[i]?.data?.eventFail && !prevById[entries[i].id]?.event
					);
					// 未发布名单按条目原始顺序合并（沿用项 + 部分列缺失项）
					const noUpdateIds = new Set([
						...retained.map((x) => x.id),
						...partialNoPrev.map((x) => x.id)
					]);
					const noUpdateNames = entries.filter((x) => noUpdateIds.has(x.id)).map((x) => x.name);
					let info = `抓取成功 ${okNames.length}/${result.total}${skippedNote}`;
					if (noUpdateNames.length > 0) info += `；${noUpdateNames.join("、")} 未发布新数据`;
					if (hardFails.length > 0) info += `；失败：${hardFails.map((x) => x.name).join("、")}`;
					setScrapeInfo(info);
					...
					open && anchor !== void 0 ? (0, react_jsx_runtime.jsx)("div", {
						...
						children: [
							...
							games.map((g) => {
								...
								const cardVisible = g.roles ? cleanRoleNames(g.roles, g.id) : (g.banner || "—");
								...
								return (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										(0, react_jsx_runtime.jsx)("div", { className: "gacha-cal-cell", title: (g.eventHover || eventTitle), children: (0, react_jsx_runtime.jsx)("span", { className: "gacha-cal-inner", children: eventName }) }),
