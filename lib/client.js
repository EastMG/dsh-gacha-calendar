window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 只取"征集时间覆盖当前时刻"的公告（多条取最新发布者）；无匹配返回 null（由官网解析兜底）。
		async function fetchXiaomiR99Gacha(gamePageUrl) {
			...
			for (let i = 0; i < marks.length; i++) {
				...
				const cand = {
					...
					roles: upM ? cleanRoles(upM[1]).replace(/[（）()].*$/, "") : "",
					order: i
				};
				// 多条覆盖当期时取最新发布（feed 靠前者更新）
		...
		async function fetchR99(listUrl, signal) {
			...
			if (!data.banner || !data.bannerDates) return null;
			// 征集名增强：小米官方资讯流有覆盖当期的征集公告时，用真实征集名/征集时间/UP 角色覆盖卡池字段
			try {
				const xiaomi = await fetchXiaomiR99Gacha("https://game.xiaomi.com/game/62346241");
