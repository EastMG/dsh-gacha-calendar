window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function bwikiCardPayload(html) {
		...
		const CARD_FETCHERS = {
			genshin: mkMediaWiki(bwikiCardPayload),
			hsr: mkMediaWiki(bwikiCardPayload),
						...
						if (r.ok && r.data) {
							...
							if (prevRec) {
								if (!merged.banner && prevRec.banner) {
									...
									if (!merged.bannerHover) merged.bannerHover = prevRec.bannerHover || "";
								}
