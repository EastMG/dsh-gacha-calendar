window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function parseAkOfficialPools(list) {
			const now = Date.now();
		...
		async function fetchArknightsOfficialPools(signal) {
			...
			const pools = parseAkOfficialPools(list);
		...
		async function fetchArknightsCard(url, signal) {
			try {
				const official = await fetchArknightsOfficialPools(signal);
