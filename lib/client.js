window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const STYLE = `
			...
			/* 截断文字悬停自动滚动显示全文（marquee 显示屏效果）：仅超宽单元格由 JS 加 gacha-cal-marq 触发 */
		...
		function CalendarPanel({ wide, scope }) {
			...
			// 截断文字悬停 marquee：内容超出容器时加 gacha-cal-marq 并计算滚动距离，
			// 让内层 span 在单元格内来回滚动显示全文（未超宽不加，保持静止）。
			(0, react.useEffect)(() => {
				const pop = popRef.current;
				if (!pop) return;
				const onOver = (e) => {
					const cell = e.target.closest(".gacha-cal-name, .gacha-cal-cell");
					if (!cell || !pop.contains(cell)) return;
				...
				pop.addEventListener("mouseover", onOver);
				pop.addEventListener("mouseout", onOut);
				return () => {
					pop.removeEventListener("mouseover", onOver);
					pop.removeEventListener("mouseout", onOut);
				};
			}, [open, anchor]);
							...
							games.map((g) => {
								...
								return (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										(0, react_jsx_runtime.jsx)("div", { className: "gacha-cal-name", title: g.name, children: (0, react_jsx_runtime.jsx)("span", { className: "gacha-cal-inner", children: g.name }) }),
										(0, react_jsx_runtime.jsx)("div", { className: "gacha-cal-cell", title: cardTitle, children: (0, react_jsx_runtime.jsx)("span", { className: "gacha-cal-inner", children: cardVisible }) }),
