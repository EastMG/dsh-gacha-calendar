window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function fetchBaGlobal(logListUrl, signal, now = Date.now()) {
			...
			const eventRow = rows
				.filter((r) => evTier(r) < 9 && activeRow(r))
				.sort((a, b) => {
					const pa = /常駐/.test(a.name) ? 1 : 0;
					const pb = /常駐/.test(b.name) ? 1 : 0;
					return (pa - pb) || (evTier(a) - evTier(b));
				})[0] || null;
			// 结束端缺失（openEnded）时只展示原文，不推算结束时间
			const evDates = eventRow ? (eventRow.openEnded ? eventRow.dateRaw : eventRow.range.raw) : "";
			const data = {
				banner: "",
				roles: "",
				bannerDates: "",
