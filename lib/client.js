window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// parseCanmoe 的宽松回退：容忍属性顺序/空白变化。仅在 parseCanmoe 未命中时使用。
		function parseCanmoeLoose(js) {
			const m = js.match(/\bd=\{(.+?)\}\s*,\s*u\s*=\s*\[/);
			if (!m) return null;
			const inner = m[1];
			const wm = inner.match(/windows\s*:\s*\[\s*\{\s*start\s*:\s*"([^"]+)"\s*,\s*end\s*:\s*"([^"]+)"\s*,\s*version\s*:\s*"([^"]+)"\s*,\s*period\s*:\s*(\d+)\s*,\s*isRerun\s*:\s*(!0|!1|true|false)\s*\}\s*\]/);
			if (!wm) return null;
			const rM = inner.match(/([^:{}]+)\s*:\s*\{\s*windows\s*:/);
			const roles = rM ? rM[1].trim() : "";
			const version = wm[3];
			const fmt = (iso) => {
				const d = new Date(iso);
				return `${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")} ${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
		...
		function CalendarPanel({ wide, scope }) {
			...
			const games = applyOrder(visible, s.order).map((g) => {
				...
				return {
					...g,
					// 单次抓取失败沿用缓存时标记该行（界面显示"缓存"并提示上次成功时间）
					_cached: !!sc.stale,
							...
							games.map((g) => {
								...
								return (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										(0, react_jsx_runtime.jsx)("div", { className: "gacha-cal-name", title: g.name + (g._cached ? `\n（上次成功 ${g._okAt ? new Date(g._okAt).toLocaleString() : "—"}）` : ""), children: (0, react_jsx_runtime.jsx)("span", { className: "gacha-cal-inner", children: g.name + (g._cached ? "（缓存）" : "") }) }),
