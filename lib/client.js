	...
	factory: (require) => {
		...
		const SOURCES = [
			...
			},
			{
				...
				source: "\u5B98\u7F51",
				// 经 host 代理 POST 抓取（官网新闻 API）；版本维护公告同时含当期卡池与活动
		...
		// 重返未来：1999 征集名对照表（官方资讯接口不发布征集名，取自官方征集公告原文；
		// 仅存"征集名"标签，不含任何日期/角色快照。新版本无记录时回退"首位6星+UP"）
		const R99_BANNER_NAMES = {
			"3.9": "烈火悬流无尽"
		};

		// 重返未来：1999（官网 re.bluepoch.com 新闻 API，POST 经 host 代理）
		// 列表接口（informationType=2 资讯）按上线时间倒序返回含全文的公告，
		// 取最新一期「版本更新维护公告」：版本名 / 当期卡池（征集名或首位6星UP）/ 当期活动 / 维护起止 + 下一期维护日
		async function fetchR99(listUrl, signal) {
			...
			// 当期卡池：征集名（对照表）优先；无记录时用新增角色首位 6 星（"6星角色「赫多涅（岩）」" → "赫多涅UP"）
			const roleM = text.match(/6星角色「([^」]+)」/);
			const verNum = verM ? String(verM[1]).match(/^(\d+\.\d+)/)?.[1] : "";
			const bannerName = (verNum && R99_BANNER_NAMES[verNum])
				? R99_BANNER_NAMES[verNum]
				: (roleM ? `${cleanRoles(roleM[1]).replace(/[（）()].*$/, "")}UP` : "");
						...
						sorted.map((g, i) => {
							...
							return (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									// 活动来源列（有活动源可切换/自定义；无活动源只读显示"未配置"）
									(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", alignItems: "center", gap: 6, justifySelf: "center", minWidth: 0 }, children: [
										...
										}) : (0, react_jsx_runtime.jsx)("span", { title: "该游戏暂无活动来源，活动列显示为空（可自行配置自定义来源）", style: { ...inputStyle, cursor: "default", color: "var(--dsw-alias-label-tertiary)" }, children: getDefaultSourceName(g, "eventUrl") }),
