	...
	factory: (require) => {
		...
		const SOURCES = [
			...
			},
			{
				...
				// 备选来源（设置页下拉可切换；值带 proxy: 前缀表示经 host 代理抓取）
				altSources: [
					{
						label: "wiki.gg",
			...
			},
			{
				...
				source: "\u5B98\u7F51",
				// 经 host 代理抓取（官网 CORS=null + 动态 SPA）
				via: "proxy",
			...
			},
			{
				...
				// 数据源为「更新日誌」board(3352)，一次请求同时含当期卡池与活动排期
				via: "proxy",
			...
			},
			{
				...
				source: "GameKee",
				// 经 host 代理抓取（GameKee 需 game-alias 头 + CDN 反爬，浏览器直连不可行）
				via: "proxy",
		...
		// 从公告 summary 提取卡池（期間 + UP 学生）——备用，主用更新日誌解析
		...
		async function proxyFetchText(proxyUrl, referer, extraHeaders) {
		...
		// 经 host 代理抓取 JSON（body 为 JSON 时）
		async function proxyFetchJson(proxyUrl, referer, extraHeaders) {
			return JSON.parse(await proxyFetchText(proxyUrl, referer, extraHeaders));
		}

		// 解析繁体中文时间："8月18日(二)維護後" / "9月1日(二)上午9點59分" / "晚間10點59分"
		function parseBaZhTime(raw, nowYear) {
			const s = String(raw).trim();
			const md = s.match(/(\d{1,2})月(\d{1,2})日/);
			if (!md) return null;
			const mo = Number(md[1]), d = Number(md[2]);
		...
		function parseBaZhRange(raw, nowYear) {
			...
			const a = parseBaZhTime(parts[0], nowYear);
			const b = parseBaZhTime(parts[1], nowYear);
			if (!a || !b) return null;
		...
		// 经代理的解析器（async）：source.via === "proxy" 或 url 以 "proxy:"/"gk-global:" 开头
		// 前缀分发：proxy: → 按 id 默认代理解析；gk-global: → GameKee 国际服
		const PROXY_PARSERS = {
			"ba-cn": (listUrl, signal) => fetchBaCn(listUrl),
			"ba-global": (listUrl, signal) => String(listUrl).startsWith("gk-global:")
				? fetchGameKeeBa("global")
				: fetchBaGlobal(listUrl, signal),
			"endfield": (proxyUrl, signal) => fetchEndfieldWikiGg(proxyUrl),
			"ba-jp": (proxyUrl, signal) => fetchGameKeeBa("jp")
		};
		//#endregion
		...
		async function fetchGame(source, signal) {
			...
			// 判断是否经 host 代理：source.via === "proxy" 或自定义 URL 带 "proxy:" 前缀（备选源）
			const proxyUrl = String(source.url ?? "").startsWith("proxy:") ? String(source.url).slice("proxy:".length) : "";
			if ((source.via === "proxy" || proxyUrl) && typeof proxyParser === "function") {
				try {
					const data = await proxyParser(proxyUrl || source.url, signal);
		...
		// 存储值约定：custom:<url>（用户自定义输入，剥前缀返回 url）、proxy:<url>（备选源，保留前缀供 fetchGame 识别）、其它原样
