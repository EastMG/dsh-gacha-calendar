	...
	factory: (require) => {
		...
		async function fetchEntry(source, signal) {
			...
			try {
				...
				let data = null;
				let cardErr = null; // 卡池源失败原因（保留，供"两者都空"时如实报告）
				...
				if (source.url) {
					if (cardFetcher) {
						...
						if (!data || !data.banner || !data.bannerDates) {
							// 自带解析未命中 → 通用解析兜底；**通用解析无产出时保留原载荷**
							// （同 URL 源的活动字段就在原载荷里，不能被覆盖成 null 一起丢掉）
							let g = null;
							try { g = await tryParseGenericCard(source.url, signal); }
							catch (err) { if (!cardErr) cardErr = err; }
							if (g) data = g;
						}
						// 卡池为空不提前判失败：若还有活动源则继续抓活动（如明日方舟跨期空档但活动仍在）
					} else {
						// 自定义条目（无内置解析器）：通用解析；**抛异常也不提前返回**，继续抓活动（卡池/活动解耦）
				...
				if (source.eventUrl) {
					...
					if (source.eventUrl === source.url && data && data.event) {
						...
						if (evName) {
							...
							// 失败原因优先报卡池源（否则卡池的真实原因会被活动源的原因盖掉）
							if (data && (data.banner || data.event)) {
								data.eventFail = gErr
									? String(gErr.message ?? gErr).slice(0, 60)
									: "no-match";
							} else {
								const why = cardErr || gErr;
								return {
									ok: false,
									reason: why ? String(why.message ?? why).slice(0, 60) : "no-match"
								};
							}
						}
					}
				...
				if (!data) {
					...
					return { ok: false, reason: cardErr ? String(cardErr.message ?? cardErr).slice(0, 60) : "no-source" };
				}
