window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 通用活动解析：扫描含「时间」表头的表格，行内找时间与名称列，选当期
		...
		async function tryParseGenericEvent(url, signal) {
			...
			if (cur && cur.banner) return { event: cur.banner, eventDates: cur.bannerDates || "" };
			return null;
		}
					...
					if (source.eventUrl === source.url && data && data.event) {
						...
						if (!evName) {
							...
							try {
								...
								if (g && g.event) { evName = g.event; evDates = g.eventDates || ""; }
							} catch (err) { gErr = err; }
						}
						if (evName) {
							data = data || {};
		...
		function CalendarSettingsPage({ scope }) {
			...
			// 添加表单临时值（仅链接：卡池来源 + 活动来源；条目数据由链接解析产出）
			const [form, setForm] = (0, react.useState)({ url: "", eventUrl: "" });
			...
			// 添加自定义条目：仅凭卡池来源链接创建（名称取链接域名，数据由刷新时解析产出）
			const addEntry = async () => {
				if (!form.url.trim()) return;
				const id = "custom-" + Date.now().toString(36);
				let name = "自定义条目";
				try { name = new URL(form.url.trim()).hostname.replace(/^www\./, ""); } catch { /* 保留默认名 */ }
				const entry = {
					id,
					name,
					icon: "",
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 6, border: "1px solid var(--dsw-alias-border-l1)", borderRadius: 10, padding: "12px 14px", background: "var(--dsw-alias-bg-layer-1)" }, children: [
						...
						adding ? (0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 8, border: "1px dashed var(--dsw-alias-border-l2)", borderRadius: 8, padding: 10, marginTop: 6 }, children: [
							...
							(0, react_jsx_runtime.jsx)("div", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 11, marginBottom: 2 }, children: "填写卡池来源链接（MediaWiki api.php 或含卡池排期的网页），刷新时自动解析；活动来源链接可选。" }),
							...
							(0, react_jsx_runtime.jsxs)("div", { style: { display: "flex", gap: 8 }, children: [
								(0, react_jsx_runtime.jsx)("button", { type: "button", className: "gacha-cal-sort-btn", disabled: !form.url.trim(), onClick: addEntry, children: "\u6DFB\u52A0" }),
