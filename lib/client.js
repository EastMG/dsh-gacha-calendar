window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 英文源 → 中文名映射（wiki.gg 是英文站，中文名以官网/bwiki 为准）
		const ENDFIELD_EN_ZH = {
			"Good Morning from Your Dawnstar": "\u300C\u6668\u661F\u4E8E\u6B64\u95EA\u8000\u300D\u7279\u8BB8\u5BFB\u8BBF",
			"Liino": "\u68A8\u8BFA",
			"Arcane": "\u5F27\u5149",
			"Camille": "\u5361\u7F2A",
			"Avywenna": "\u827E\u7EF4\u6587\u5A1C"
		};
		function endfieldZh(en) {
			return ENDFIELD_EN_ZH[en] || en;
		}
		function parseEndfieldCurrent(html) {
			...
			const banner = nameM ? endfieldZh(nameM[1].trim()) : "";
			...
			return {
				banner,
				version: "",
				roles: [...new Set(upM.map((m) => endfieldZh(m[1])))].join("、"),
