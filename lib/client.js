			...
			const doRefresh = (0, react.useCallback)(async () => {
				...
				try {
					...
					const eventFails = result.results
						...
						.filter(Boolean);
					// 通用提示分类（不针对任何游戏特判）：
					// - 沿用：本次未产出新数据，但此前有成功记录（原数据照常展示、标 stale、okAt 不刷新）
					//   → 归为"无新当期（沿用最近成功数据）"，不再显示成"失败"
					// - 真失败：未产出数据且此前也没有成功记录（面板无任何可显示内容）→ "失败"
					...
					if (retained.length > 0) info += `；${retained.map((x) => x.name).join("、")} 未发布新数据`;
