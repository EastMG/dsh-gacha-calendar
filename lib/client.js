window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 时间段统一格式 mm-dd hh:mm ~ mm-dd hh:mm（无法解析的一侧保留原文）
		function formatRange(raw) {
			const t = stripTags(raw);
			const parts = t.split(/~/).map((x) => x.trim());
			if (parts.length < 2) return t;
			const a = formatDate(parts[0]);
			const b = formatDate(parts[1]);
			if (a && b) return `${a} ~ ${b}`;
			if (!a && b) return `${parts[0]} ~ ${b}`;
			if (a && !b) return `${a} ~ ${parts[1]}`;
			return t;
		}

		// bwiki 通用：第一张含「时间+版本」的卡池表（原神/星铁/绝区零）
		function parseBwiki(html) {
			...
			for (const t of tables) {
				...
				return {
					banner,
					bannerDates: timeM ? formatRange(timeM[1]) : "",
		...
		function parseArknights(html) {
			...
			const roles = tds.length > 2
				? [...tds[2].matchAll(/<a[^>]*href="\/w\/[^"]*"[^>]*title="([^"]*)"/g)]
					...
					.join("、")
				: "";
			return { banner, bannerDates, version: "限时寻访", roles };
		}
		...
		function CalendarPanel({ wide, scope }) {
			...
			// 抓取成功的数据覆盖内置的卡池列（角色名拼进卡池名），活动列保持内置
			const games = applyOrder(SOURCES, s.order).map((g) => {
			...
			const doRefresh = (0, react.useCallback)(async () => {
				...
				try {
					...
					result.results.forEach((r, i) => {
						if (r.ok && r.data) dataById[SOURCES[i].id] = r.data;
					});
					...
					const okNames = SOURCES.filter((x, i) => result.results[i]?.ok).map((x) => x.name);
