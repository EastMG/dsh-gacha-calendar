window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function parseWuwaCalendar(html) {
			...
			if (!cur) return null;
			return { banner: cur.banner, roles: "", bannerDates: cur.bannerDates };
		}
		...
		// 依次尝试 bwiki 式「时间+版本」表、方舟式「限时寻访」表、GachaTracker 式日期表，选当期；
		// 全部失败返回 null（调用方回退健康检查/用户填写内容）
		...
		async function fetchEntry(source, signal) {
			...
			try {
				...
				let data = null;
				// —— 卡池字段：有解析器 → 抓取解析；无内置解析器（自定义条目）→ 尝试通用解析，失败仅健康检查 ——
				if (source.url) {
					if (cardFetcher) {
						...
						try {
							const g = await tryParseGenericCard(source.url, signal);
							if (g && g.banner && g.bannerDates) {
								data = g;
							} else {
								// 通用解析失败：健康检查兜底，HTTP 200 即联网成功
		...
		function CalendarSettingsPage({ scope }) {
			...
			// 添加表单临时值
			const [form, setForm] = (0, react.useState)({ name: "", icon: "", banner: "", bannerDates: "", event: "", eventDates: "", url: "", eventUrl: "" });
			...
			const addEntry = async () => {
				...
				setForm({ name: "", icon: "", banner: "", bannerDates: "", event: "", eventDates: "", url: "", eventUrl: "" });
				setAdding(false);
			};
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("div", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: 12 }, children: "数据来源：官方公告 / 官方Wiki（bwiki、PRTS 等）；无联网抓取数据时对应列显示为空。自定义条目的卡池来源地址会尝试解析（MediaWiki/常见卡池表格），解析失败时展示你填写的内容。" })
				]
			});
		}
		//#endregion
