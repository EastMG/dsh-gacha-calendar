	...
	factory: (require) => {
		...
		const SOURCES = [
			...
			},
			{
				...
				altSources: [
					{ label: "GameKee 当期卡池", value: "gk-jp:", fetcher: "ba-jp-gamekee" }
				],
				...
				eventAltSources: [
					{ label: "官方公告（日文）", value: BA_JP_NEWS_URL, fetcher: "ba-jp-official" }
				]
			},
			{
				id: "r1999",
		...
		async function fetchBaJpCard(url, signal, now = Date.now()) {
			try {
		...
		const CARD_FETCHERS = {
			...
			"ba-jp": (url, signal) => fetchBaJpCard(url, signal),
			"ba-jp-gamekee": mkProxy(() => fetchGameKeeBa("jp")),
