	...
	factory: (require) => {
		...
		const SOURCES = [
			...
			},
			{
				...
				// 独立活动源：Game8（英文，国际服活动排期，经 host 代理；当期并行活动选结束最晚）
				eventUrl: "https://game8.co/games/Arknights-Endfield/archives/535443",
				eventSource: "Game8\uff08\u82F1\u6587\uff09"
			},
			{
				id: "ba-cn",
				name: "蔚蓝档案·国服",
			...
			},
			{
				...
				source: "LDSHOP",
				// 经 host 代理抓取（ldshop 无 CORS 头）；静态 HTML 表格，含当期+下一期卡池（期間/角色/棋盤）
				url: "https://www.ldshop.gg/tw/blog/nte/neverness-to-everness-banner.html",
				eventUrl: ""
			}
		];
		//#endregion
		...
		function parseArknights(html) {
			...
			for (const rm of body.matchAll(/<tr>([\s\S]*?<td[\s\S]*?)<\/tr>/g)) {
				...
				out.push({ banner, roles, ...range, isMain: true });
			}
			return out;
		}
		...
		function parseCanmoe(js) {
			const curM = js.match(/\bd=\{(.+?)\},\s*u=\[/);
			if (!curM) return null;
			const inner = curM[1];
			const wm = inner.match(/windows:\[\{start:"([^"]+)",end:"([^"]+)",version:"([^"]+)",period:(\d+),isRerun:(!0|!1)\}\]\}/);
			if (!wm) return null;
			...
			return {
				banner: `\u3010${version}\u3011${roles}`,
				roles,
				bannerDates: `${fmt(wm[1])} ~ ${fmt(wm[2])}`
			};
		}
		...
		async function fetchYsActivity(signal) {
			const q = "[[\u5206\u7C7B:\u6D3B\u52A8]][[\u7ED3\u675F\u65F6\u95F4::>2026/01/01]]|?\u540D\u79F0|?\u5F00\u59CB\u65F6\u95F4|?\u7ED3\u675F\u65F6\u95F4|sort=\u5F00\u59CB\u65F6\u95F4|order=desc|limit=60";
		...
		const CARD_FETCHERS = {
			...
			nte: mkProxy((url, signal) => fetchLdshopNte(url)),
		...
		const EVENT_FETCHERS = {
			...
			// 终末地：Game8（英文，经 host 代理；当期并行活动选结束最晚）
			endfield: {
				default: (url, signal) => fetchGame8Endfield(url).then((d) =>
		...
		const STYLE = `
			...
			.gacha-cal-pop{position:fixed;z-index:9999;width:660px;max-height:72vh;overflow:auto;background:var(--dsw-alias-bg-base);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,.25);padding:10px 12px;font-size:12px;color:var(--dsw-alias-label-primary)}
			...
			.gacha-cal-row{display:grid;grid-template-columns:22px 96px 1.2fr 165px 1fr 165px;gap:6px;align-items:center;padding:5px 4px;border-bottom:1px solid var(--dsw-alias-border-l1)}
			...
			.gacha-cal-h{display:grid;grid-template-columns:22px 96px 1.2fr 165px 1fr 165px;gap:6px;align-items:center;padding:6px 4px 2px;color:var(--dsw-alias-label-tertiary);font-size:11px}
							...
							games.map((g) => {
								...
								return (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										(0, react_jsx_runtime.jsx)("div", { className: "gacha-cal-cell", title: g.bannerDatesRaw || g.bannerDates, children: (0, react_jsx_runtime.jsx)("b", { children: displayTimeCell(g.bannerDates || "", now) }) }),
										(0, react_jsx_runtime.jsx)("div", { className: "gacha-cal-cell", title: eventTitle, children: eventName }),
										(0, react_jsx_runtime.jsx)("div", { className: "gacha-cal-cell", title: g.eventDatesRaw || g.eventDates, children: displayTimeCell(g.eventDates || "", now) })
									]
								});
							})
						]
