...
- **真实抓取驱动面板**（v0.3.0 / v0.3.8 / v0.4.0）：点击刷新后，原神/星铁/绝区零/明日方舟/鸣潮/终末地/**蔚蓝档案国际服** 的**卡池名与起止时间直接来自 wiki/官方公告实时解析**（确定性纯函数解析，同一网页内容每次刷新结果一致），日期统一为 `mm-dd hh:mm ~ mm-dd hh:mm`；鸣潮的活动列也实时抓取（bwiki 活动日历）；其余游戏无公开可抓源，保持内置数据
- **Host 同源代理**（v0.4.0）：对 CORS/Referer 反爬的官方源（如 Nexon 官方论坛），host 端注册同源 HTTP 代理路由（`/api/gacha-calendar-proxy`，白名单域名 + Referer 覆盖），浏览器端经代理抓取绕开浏览器限制——蔚蓝档案国际服当期卡池即来自 **Nexon 官方公告**（公告列表 → 详情 summary → 卡池名/UP 学生/起止时间）
