...
DeepSeek Harness 桌面端侧边栏插件：一键查看已安装二游的**当期卡池与活动起止**，支持联网自动刷新。

## 功能

- 侧边栏底部「📅 二游排期」按钮 → 悬浮面板，按行展示每款游戏的当期卡池、卡池起止、当期活动、活动起止
- **倒计时显示**剩余时间，悬停查看卡池/活动全名与源站原始时间文本
- **联网抓取**：各游戏来源独立（官网公告 / Wiki / 第三方站），失败自动降级；来源可在设置页切换或自定义地址
- **设置页**：自动刷新开关与频率（1–42 天）、展示顺序、条目显隐/删除、新增自定义条目（填名称+图标+来源链接，内容由链接解析产出）
...
排期数据实时抓取自各游戏**官方公告 / Wiki / 第三方站**（非内置静态数据）。来源带语言后缀标注，如 `Game8（英文）`；繁体中文源不加后缀。
