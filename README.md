...
- **真实抓取驱动面板**（v0.3.0）：点击刷新后，原神/星铁/绝区零/明日方舟 的**卡池名与起止时间直接来自 wiki 实时解析**（确定性纯函数解析，同一网页内容每次刷新结果一致），日期统一为 `mm-dd hh:mm ~ mm-dd hh:mm`；其余游戏无公开可抓源，保持内置数据
...
- **条目管理**（v0.3.2 / v0.3.3）：设置页「条目管理」卡片整合**展示顺序与条目管理**——每行可勾选「展示」控制面板显隐、↑↓ 调整顺序、选择**默认源（显示源名如 Bwiki / PRTS Wiki / 内置数据）或自定义爬取地址**、🗑 图标删除条目（内置条目记入删除列表可一键恢复）；并支持**添加自定义条目**（填名称/版本/卡池/活动/图标即可显示在面板，可选填爬取地址做联网检查）
